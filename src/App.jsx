...
function App() {
    ...
    const addToCart = (product) => {
        setCart(prevCart => {
            ...
            if (existing) {
                // Met à jour la quantité de l'item existant
        ...
        <div>
            ...
            <div className="container">
                ...
                <div className='items-left'>
                    <h2>Your cart <span>{cart.reduce((total, item) => total + item.quantity, 0)}</span></h2>

                    {cart.length === 0 ? (
                        // ✅ Affiché seulement quand le panier est vide
                        ...
                        // ✅ Affiché quand il y a des articles
                        <>
                        <ul className="list-card">
                            {cart.map((item, index) => (
                                <li key={index}>
                                    <span >{item.name}</span>
                                    <span>{item.quantity}x </span> {item.price * item.quantity}
                                    <a href="#" onClick={(e) => {
